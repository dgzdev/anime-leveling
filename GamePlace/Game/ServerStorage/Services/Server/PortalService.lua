...
function PortalService:HasPortalClose(Position: Vector3, Radius: number)
	...
	local JoinPortal = Workspace.City.JoinPortal
	...
	if JoinPortal and (Position - JoinPortal:GetPivot().Position).Magnitude <= Radius then
		return true
	end
...
function PortalService.Client:Teleport(player)
	return self.Server:Teleport(player)
end
