...
function UIDebounceController:UpdateDebounces(DebounceData: {Duration: number, Name: string})
    ...
    if HaveDebounce then
        ...
        Tweens[Name] = TweenService:Create(Placeholder.Cooldown, TweenInfo.new(Duration, Enum.EasingStyle.Linear), {Size = UDim2.new(0,0,1,0)})
    end
...
function UIDebounceController.KnitInit()
