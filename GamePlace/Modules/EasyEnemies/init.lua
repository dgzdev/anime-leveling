...
type e_settings =
	...
	& ATTACK_NPCS
	& ATTACK_PLAYERS
	& DEFAULT_ATTACK_AIMATIONS
	& DEFAULT_ATTACK_FUNCTIONS
	& SPECIAL_ATTACK_AIMATIONS
	& SPECIAL_ATTACK_FUNCTIONS
...
function SettingsCheck(current_settings: e_settings)
	for i, setting: string in settings_list do
		if current_settings[setting] == nil then
			warn(
				string.format(
					"Check your enemy settings, %q is nil, setting to default value: %i",
					setting,
...
function Enemy.new(object: string | Instance, _settings: e_settings)
	...
	if object_type == "string" then
		for _, instance: Instance in (CollectionService:GetTagged(object)) do
