...
function CLASS.new()
	...
	local dataTable = setmetatable({
		...
		--// Configurations //--
		VerticalAngleLimits = NumberRange.new(-70, 70),
		----
...
function CLASS:SetShoulderDirection(shoulderDirection)
	...
	assert(
		...
		"OTS Camera System Argument Error: Attempt to set unrecognized shoulder direction " .. shoulderDirection
	)
	if self.IsEnabled == false then
...
function CLASS:Update()
	...
	self.VerticalAngle =
		math.rad(math.clamp(math.deg(self.VerticalAngle), self.VerticalAngleLimits.Min, self.VerticalAngleLimits.Max))
	----
